<template>
    <div
        class="mb-8 mx-auto mt-12 max-w-md bg-gradient-to-r from-blue-600 via-indigo-700 to-purple-800 text-white rounded-lg shadow-xl p-8">
        ...
        <form class="mt-8 space-y-6" @submit.prevent="handleLogin">
            ...
            <div>
                <button type="submit"
                    class="w-full bg-blue-700 text-white py-2 rounded-md hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-400 transition duration-200 ease-in-out transform hover:scale-105">Đăng
                    Ký</button>
            </div>
...
const handleLogin = async () => {
    try {
        ...
        if (res.data.value.token && res.data.value.user) {
            ...
            userStoreRef.login();
            navigateTo('/').then(() => {
                useNuxtApp().$toast.success('Đăng ký thành công');
            })

        }
    } catch (error) {
        useNuxtApp().$toast.error('Đã xảy ra lỗi trong quá trình đăng ký');
    }
